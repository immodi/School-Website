        ...
        <ul class="collapse list-unstyled" id="zoomMenu">
            @if(userPermission(555) && menuStatus(555))
                <li data-position="{{menuPosition(555)}}">
                    <a href="{{ route('zoom.virtual-class')}}">@lang('zoom::lang.virtual_class')</a>
                </li>
            @endif
            @if(userPermission(560) && menuStatus(560))
                <li data-position="{{menuPosition(560)}}">
                    <a href="{{ route('zoom.meetings') }}">@lang('zoom::lang.virtual_meeting')</a>
                </li>
            @endif
            @if(userPermission(565) && menuStatus(565))
                <li data-position="{{menuPosition(565)}}">
                    <a href="{{ route('zoom.virtual.class.reports.show') }}">@lang('zoom::lang.class_reports')</a>
                </li>
            @endif
            {{-- @if(userPermission(565) && menuStatus(565))
            <li data-position="{{menuPosition(565)}}">
                <a href="{{ route('zoom.virtual.class.reports.show') }}">@lang('zoom::lang.Recorder') @lang('zoom::lang.file')</a>
            </li>
            @endif --}}
            ...
            @if(userPermission(567) && menuStatus(567))
                <li data-position="{{menuPosition(567)}}">
                    <a href="{{ route('zoom.meeting.reports.show') }}">@lang('zoom::lang.meeting_reports')</a>
                </li>
            @endif
            @if(userPermission(569) && menuStatus(569))
                <li data-position="{{menuPosition(569)}}">
                    <a href="{{ route('zoom.settings') }}">@lang('zoom::lang.settings')</a>
                </li>
            @endif
        </ul>
    </li>
